...
| App (f, g) as app ->
    ...
    | TFun (in_t, out_t) ->
        if type_match b tenv g_t in_t then out_t
        else TBad app
    | TPi (x, t, body) ->
        if type_match b tenv g_t t then sub_term_type g x body
        else TBad app
    | _ -> TBad app
    end
...
| TBool ->
    begin match t with
    | TVar v -> env_lookup_match b tenv v TBool
    | _ -> t = TBool
    end
| TInt ->
    begin match t with
    | TVar v -> env_lookup_match b tenv v TInt
    | _ -> t = TInt
    end
...
| TPi (x, t', body) as self->
    ...
    | TPi (y, t'', body') ->
        type_match b tenv t'' t' && type_match b tenv body (sub_term_type (Var x) y body')
