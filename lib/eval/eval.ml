...
| App (Lam (x, t, body), g) ->
    ...
    | TVar v ->
        ...
        | Some v_t ->
            if v_t = g_t then eval venv' tenv body
            else illtyped g v_t
        | None -> eval venv' tenv body
        end
    | _ ->
        if t = g_t then eval venv' tenv body
        else illtyped g t
    end
| App (_, _) as app ->
    let app' = reduce tenv app in
    if app' = app then Virreducible app
    else eval venv tenv app'
| BLit b -> Vbool b
...
| App (f, g) ->
    begin match reduce tenv f with
